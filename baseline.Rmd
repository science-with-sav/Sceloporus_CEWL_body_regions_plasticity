...
```{r}
all_data_wide %>%
  ggplot(., aes(x = RH_percent_interpol)) +
  geom_histogram(color = "black", fill="steelblue", bins=30) + 
  theme_classic() +
  xlab("Humidity") + 
  ylab("Percentage")
simple.eda(all_data_wide$RH_percent_interpol)
# Normality test if p > .05, data is normal. Data is not normal
shapiro.test(all_data_wide$RH_percent_interpol)

# Doesn't fix non normality
shapiro.test(log(all_data_wide$RH_percent_interpol))
...
@ dylan I also didn't test any of the temp/humidity variables for normality...

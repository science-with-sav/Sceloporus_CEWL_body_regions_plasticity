...
# glm
glm_hct_sex <- lm(hematocrit_percent ~ sex_M_F,
           data = all_data_wide)
summary(glm_hct_sex)
```
...
# glm
glm_osml_wk <- lm(osmolality_mmol_kg ~ date,
           data = all_data_wide)
summary(glm_osml_wk)
```
...
# glm
glm_osml_RH <- lm(osmolality_mmol_kg ~ RH_percent_interpol,
           data = all_data_wide)
summary(glm_osml_RH)
```
...
The best model to predict hydration is based on ambient temperature at capture as a fixed effect and individual_ID as a random effect. Using the interaction between RH * temperature gives just as good a model.
...
Step 6 is the final, best model, but step 9 was a close runner-up. Save the output from both as dataframes and export.

```{r}
# save step 6 summary object
CEWL_best_mod <- summary(CEWL_mod9_step6)
# extract stats table from summary object
CEWL_best_mod_vals <- data.frame(CEWL_best_mod$tTable)
# export 
write.csv(CEWL_best_mod_vals, "CEWL_best_mod_vals.csv")

# save step 9 summary object
CEWL_best_mod2 <- summary(CEWL_mod9_step9)
# extract stats table from summary object
CEWL_best_mod2_vals <- data.frame(CEWL_best_mod2$tTable)
# export 
write.csv(CEWL_best_mod2_vals, "CEWL_2nd_best_mod_vals.csv")
```

...
- normal: only variable to pass normality test (LOL)
```{r}
all_data_wide %>%
...
This will be important to keep in mind going forward. Once we have our model, we will need to vigorously check assumptions.

## Transformations

I'm just going to start with simple log-transformations and hope that improves normality, then investigate potential other transformations if log-transforming does not work.
...
### Check LM Assumptions (Hydration Model 5)
...
It doesn't look like there's a clear linearity violation because there isn't a shape/pattern to the residuals, however the groupings makes it look like error variance is not equal. 

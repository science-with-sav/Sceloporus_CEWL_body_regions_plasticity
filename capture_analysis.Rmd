...
morpho_blood_SMI %>% 
  ...
  stat_smooth(aes(x = VPD_kPa_int, 
              ...
              alpha = 1 ) + 
  theme_classic() + 
  xlab("Vapor Pressure Deficit at Capture (kPa)") + 
  #ylab("Osmolality (mmol/kg)") + 
  ylab("") +
  xlim(0, 2) +
  ylim(300, 450) +
...
# lm
lm_osml_vpd <- lm(osmolality_mmol_kg ~ VPD_kPa_int,
           data = morpho_blood_SMI)
summary(lm_osml_vpd)
```
...
# lm
lm_hct_VPD <- lm(hematocrit_percent ~ VPD_kPa_int,
           data = morpho_blood_SMI)
summary(lm_hct_VPD)
```
...
- capture temp & VPD (both ***)

So, for the LMM to predict osmolality, we will start with SMI, hemolyzed/not, week/date, and capture temp and VPD as our predictor variables in the model. 
...
```{r}
CEWL_lm6 <- lm(TEWL_g_m2h ~ region * VPD_kPa_int,
           data = CEWL_data_full)
summary(CEWL_lm6)
```
...
Based on the simple linear models and figures above, osmolality should be predicted by sample eye, hemolysis, date/week, individual, capture temperature, and capture VPD.
...
Date and individual_ID are collinear and should not both be used as fixed effects. But, individual_ID is a factor and there is not repetition in the dataset, so that's not feasible as a variable. Temperature and VPD are the only continuous variables, and they are not badly collinear.
...
Based on AIC, dropping SVL, hold time, temp, VPD, wind speed, region:ambient_temp_C, and temp_C_interpol:VPD_kPa_int would result in a better model. 

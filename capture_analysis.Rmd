...
Date and individual_ID are collinear and should not both be used as fixed effects. But, individual_ID is a factor and there is not repetition in the dataset, so that's not feasible as a variable. Temperature and VPD are the only continuous variables, and they are very badly collinear.

Try centering VPD and temp:

```{r}
hydrat_mod_dat_c <- hydrat_mod_dat %>%
  mutate(VPD_mean = mean(VPD_kPa_int),
         temp_mean = mean(temp_C_interpol),
         VPD_c = VPD_kPa_int - VPD_mean,
         temp_c = temp_C_interpol - temp_mean)
summary(hydrat_mod_dat_c)
```
...
Dropping terms will not meaningfully improve the model based on AIC... 

Re-run model using lmertest to get p-values:

...
unique(CEWL_data_full$individual_ID)
```
...
I want to save the data as a csv for loading into other analyses.

```{r}
write.csv(CEWL_data_full, "exported_data/capture_data_long.csv")
```
...
lm_hct_sex_mass <- glm(hematocrit_percent ~ sex_M_F*mass_g - mass_g,
           data = morpho_blood_SMI)
...
morpho_blood_SMI %>% 
  ...
  annotate("text", x = 8.5, y = 440, 
           label = "paste(italic(R) ^ 2, \" = 0.038\")", 
           parse = TRUE,
           size = 6) +
  annotate("text", x = 8.5, y = 428, 
           label = "paste(italic(p), \" = 0.011\")", 
           parse = TRUE,
           size = 6) +
  theme(text = element_text(color = "black", 
                            family = "sans", 
...
osml_best_mod <- summary(hydrat_mod2)
...
CEWL_mod1 <- lme4::lmer(data = CEWL_mod_dat,
                        TEWL_g_m2h ~ region + 
                         region * mass_g +
                         region * ambient_temp_C +
                         region * temp_C_interpol +
...
ambient_temp_C, Solar_rad_Wm2_interpol, and region:mass_g could potentially be dropped without worsening the model.

Start with ambient_temp_C since there is another temperature variable included:
...
```
We do lose significant predictive power when we go from model 3 to 4... 
...
# compare
anova(CEWL_mod6, CEWL_mod5)
anova(CEWL_mod6, CEWL_mod1)
```
...
```{r}
# save best model summary object
CEWL_best_mod <- summary()
# extract stats table from summary object

---
title: "Cal Poly Herpetology CURE 2021 - Capture Data Analyses"
...
#### Stats for CDFW Permit Report

```{r}
permit_stats <- morpho_blood_dat %>%
  group_by(date, sex_M_F) %>%
  summarise(n = n())
permit_stats
# check total
sum(permit_stats$n)
...
CEWL_means <- CEWL %>%
  group_by(region) %>%
  summarise(n_obs = n(),
            mean_CEWL = mean(TEWL_g_m2h),
            SD_CEWL = sd(TEWL_g_m2h),
            SEM_CEWL = SD_CEWL/sqrt(n_obs)
            ) %>%
  dplyr::select(region, mean_CEWL, SEM_CEWL)
  #arrange(mean_CEWL)
CEWL_means
...
```{r}
ggarrange(osml_wind_fig, osml_vpd_fig, 
          osml_sorad_fig, osml_SMI_fig, 
          ncol = 2, nrow = 2,
          labels = c("A", "B", "C", "D"),
          hjust = 0, vjust = 1.1,
...
```{r}
ggarrange(cap_vpd_CEWL_fig, msmt_VPD_CEWL_fig, 
          cap_temp_CEWL_fig, msmt_temp_CEWL_fig, 

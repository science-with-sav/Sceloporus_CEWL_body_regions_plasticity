...
mass_SVL_SLR_residuals <- morpho_blood_dat %>%
  ...
  mutate(#y_hat = predict(mass_SVL_SLR),
         ...
         y_hat = 0.36921*SVL_mm - 13.59918,
         e = y_hat - mass_g
         ) %>%
  dplyr::filter(e < 2.8 & e > -3.5)
```
...
Not zero, which is not ideal. Next, check for influential points.
...
hydrat_mod_dat %>%
  group_by(blood_sample_eye) %>%
  dplyr::summarize(mean(osmolality_mmol_kg), max(osmolality_mmol_kg), min(osmolality_mmol_kg))
```
...
Once again, the AIC is slightly lower, and our model fit is not significantly worse by going to model 3.

...
osml_d0 <- all_dat %>%
  dplyr::filter(day == 0) %>%
  dplyr::select(individual_ID, osml_day0 = osmolality_mmol_kg, 
                humidity_tmt_percent)
osml_d8 <- all_dat %>%
  dplyr::filter(day == 8) %>%
  dplyr::select(individual_ID, osml_day8 = osmolality_mmol_kg, 
                humidity_tmt_percent)

osml_diffs <- osml_d0 %>%
  left_join(osml_d8, by = c('individual_ID', 'humidity_tmt_percent'))
```

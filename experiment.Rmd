...
mass_SVL_SLR_residuals <- all_dat %>%
  ...
  mutate(y_hat = 0.36921*SVL_mm - 13.59918,
         e = y_hat - mass_g) %>%
  dplyr::filter(abs(e) < 2.2)
```
...
From the histogram, it looks like there are some points > ~2.2 that are outside the main bell curve, so go back and try deleting those points. This definitely made it look better. 

However, the boxplot version had no extra dots, suggesting none of the omitted points were actually outliers, so don't omit data for next steps. 
...
```{r}
log_mass_SVL_SLR <- lm(data = all_dat, 
...
```{r}
r <- sqrt(0.5312) # Pearson's correlection coefficient (sqrt of R-squared)
b_OLS <- 2.4275 # regression slope
b_SMA <- b_OLS/r
```
...
```{r}
mass_mod <- lme4::lmer(data = all_dat,
...
*Q*: is there a way to get each individual as a line, but to color lines based on treatment, rather than individual ID?

model:

```{r}
SMI_mod <- lme4::lmer(data = scaled_mass_index,
...
*Q*: I think this is the line if all the points were aggregated... how do I get the mean line?? i.e. I want the mean slope to make a line.
...
CEWL %>% 
  ...
  theme(text = element_text(color = "black", 
        ...
        axis.text = element_text(color = "black", 
                                 ...
                                 size = 10),
        legend.text.align = 0,
        legend.position = "none" # too many thngs in it
)
```

Okay, now is ther any way to color by treatment?!


```{r}
CEWL_mod <- CEWL %>%
  dplyr::filter(complete.cases(CEWL)) %>%
  lme4::lmer(data = .,
...
The model is best with all the parameters currently included. 

...
SMI_diff_lm <- all_dat_no_rehab %>%
  dplyr::filter(day == "Before Experiment") %>%
...
SMI_diff_lm_end <- all_dat_no_rehab %>%
  dplyr::filter(day == "After Experiment") %>%
  lm(data = ., SMI ~ humidity_tmt_percent)
...
```{r}
SMI_mod_final <- lmerTest::lmer(data = all_dat_no_rehab,
                                SMI ~ day + humidity_tmt_percent +
                                  (1|trial_number))
write.csv(broom.mixed::tidy(SMI_mod_final), 
...
L & E both seem fine. Slight fanning to the right, but otherwise no big pattern, so L is definitely okay. E could use help.
...
### Export Best
```{r}
hct_mod_final <- all_dat_no_rehab %>%
  dplyr::filter(complete.cases(hematocrit_percent)) %>%
  lmerTest::lmer(data = .,
               hematocrit_percent ~ day +
               (1|trial_number))
write.csv(broom::tidy(hct_mod_final), 
...
```{r}
simple.eda(residuals(osml_mod))
shapiro.test(residuals(osml_mod))
```
...
```{r}
osml_mod_final <- all_dat_no_rehab %>%
  dplyr::filter(complete.cases(osmolality_mmol_kg)) %>%
  lmerTest::lmer(data = .,
               osmolality_mmol_kg ~ day * humidity_tmt_percent +
               (1|trial_number))
write.csv(broom::tidy(osml_mod_final), 
...
drop1(CEWL_mod3_t)
plot(CEWL_mod3_t)
simple.eda(residuals(CEWL_mod3_t))
shapiro.test(residuals(CEWL_mod3_t))
```
...
CEWL_mod_final <- CEWL %>%
  ...
  lmerTest::lmer(data = .,
               ...
               day*humidity_tmt_percent + region + cloacal_temp_C +
               (1|trial_number/individual_ID))
write.csv(broom::tidy(CEWL_mod_final), 
